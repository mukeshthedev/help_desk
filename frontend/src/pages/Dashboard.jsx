    ...
    <div className="card" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <div style={{ fontSize: '2.2rem', fontWeight: 800, color: 'var(--text-primary)', lineHeight: 1 }}>{value ?? '—'}</div>
          {sub && <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.4rem' }}>{sub}</div>}
        </div>
        <div style={{ width: 44, height: 44, background: `${color}20`, borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', color }}>
          <Icon size={20} />
        </div>
      </div>
    </div>
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem' }}>
        ...
        <Link to="/submit" className="btn btn-primary"><Plus size={16} />New Complaint</Link>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1.25rem', marginBottom: '1.5rem' }}>
        <div className="card" style={{ gridColumn: 'span 2' }}>
          ...
          {top5.length === 0 ? <div className="empty-state">No data yet. Submit some complaints first.</div> : (
                  ...
                  <div key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.35rem' }}>
                      <span style={{ fontSize: '0.875rem', color: 'var(--text-secondary)' }}>
                        <span style={{ display: 'inline-block', width: 22, height: 22, borderRadius: 6, background: `${COLORS[i]}20`, color: COLORS[i], textAlign: 'center', lineHeight: '22px', fontSize: '0.7rem', fontWeight: 700, marginRight: '0.5rem' }}>#{i + 1}</span>
                        {item._id}
                      </span>
                      <span style={{ fontSize: '0.875rem', fontWeight: 600, color: COLORS[i] }}>{item.count} <span style={{ color: 'var(--text-muted)', fontWeight: 400 }}>({pct}%)</span></span>
                    </div>
        ...
        <div className="card">
          ...
          {priorityData.length > 0 ? (
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={priorityData} dataKey="count" nameKey="_id" cx="50%" cy="50%" innerRadius={50} outerRadius={80} paddingAngle={3}>
                  {priorityData.map((entry, i) => <Cell key={i} fill={PRIORITY_COLORS[entry._id] || COLORS[i]} />)}
                </Pie>
          ...
          {catData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={catData} margin={{ left: -20 }}>
                <XAxis dataKey="_id" tick={{ fill: 'var(--text-muted)', fontSize: 10 }} angle={-30} textAnchor="end" height={50} />
          ...
          {deptData.length > 0 ? (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={deptData} layout="vertical" margin={{ left: 10 }}>
                ...
                <YAxis dataKey="_id" type="category" tick={{ fill: 'var(--text-secondary)', fontSize: 12 }} width={80} />
                <Tooltip contentStyle={{ background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 8, fontSize: 12 }} />
